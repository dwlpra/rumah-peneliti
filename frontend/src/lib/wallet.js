...
export function WalletProvider({ children }) {
  ...
  const switchNetwork = async () => {
    ...
    try {
      await window.ethereum.request({ method: "wallet_switchEthereumChain", params: [{ chainId: ZERO_G_CHAIN_ID }] });
    } catch(e) {
      if (e.code === 4902) {
  ...
  return (
    <WalletContext.Provider value={{ address, provider, signer, connect, disconnect, balance, chainId, isCorrectNetwork, switchNetwork }}>
      {children}
