    ...
    <Card className="h-full flex flex-col">
      <CardContent className="flex-1 flex flex-col p-5 space-y-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-muted-foreground">{t("card_model")}</span>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="font-medium">{t("card_performance")}</span>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-lg font-bold">{stats.papers_curated || 0}</p>
              <p className="text-[10px] text-muted-foreground">{t("card_papers_curated")}</p>
            </div>
            <div>
              <p className={`text-lg font-bold ${getScoreColor(stats.avg_score || 0)}`}>
                {stats.avg_score || 0}
              </p>
              <p className="text-[10px] text-muted-foreground">{t("card_avg_score")}</p>
            </div>
          </div>
          ...
          {(stats.avg_novelty > 0 || stats.avg_clarity > 0 || stats.avg_methodology > 0 || stats.avg_impact > 0) && (
            <div className="space-y-2">
              {[
                { label: t("card_novelty"), value: stats.avg_novelty || 0 },
                { label: t("card_clarity"), value: stats.avg_clarity || 0 },
                { label: t("card_methodology"), value: stats.avg_methodology || 0 },
                { label: t("card_impact"), value: stats.avg_impact || 0 },
              ].map((dim) => (
                <div key={dim.label}>
          ...
          {stats.last_activity && (
            <p className="text-[10px] text-muted-foreground">
              {t("card_last_active")} {new Date(stats.last_activity).toLocaleDateString()}
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="font-medium">{t("card_tip_agent")}</span>
            {Number(tips.totalTips || 0) > 0 && (
              <span className="ml-auto text-[10px] font-mono">
                {Number(tips.totalTips).toFixed(4)} 0G {t("card_earned")}
              </span>
            )}
          </div>
          ...
          >
            {tipping ? (
              ...
              <>
                <Coffee className="h-3 w-3" />
                {t("card_tip")} {TIP_AMOUNTS[selectedTip].label} 0G
              </>
            )}
          </Button>

          <p className="text-[10px] text-muted-foreground text-center">
            {t("card_funds_compute")}
          </p>
        </div>
        ...
        {agent.agenticId && (
          <div className="rounded-md border border-blue-500/20 bg-blue-500/5 p-2.5 space-y-1.5">
            ...
            <div className="text-[10px] text-muted-foreground space-y-0.5">
              <p>Token #{agent.agenticId.tokenId} • {agent.agenticId.intelligentData?.length || 0} {t("card_onchain_entries")}</p>
              <p>{t("card_model")} {agent.agenticId.model || agent.agenticId.agentType}</p>
            ...
            >
              {t("card_verify_explorer")} <ExternalLink className="h-2.5 w-2.5" />
            </a>
          </div>
        )}
        ...
        <div className="mt-auto pt-2 flex items-center gap-3">
          ...
          >
            {t("card_agentic_id")} <ExternalLink className="h-3 w-3" />
          </a>
        </div>
      </CardContent>
    </Card>
